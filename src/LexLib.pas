...
procedure fatal ( msg : String );
	...
	begin
		raise Exception.Create('LexLib: ' + msg);
	end(*fatal*);
...
eoi					: Integer;
bufptr      : Integer;
ungetbuf    : array [0..max_chars] of Char;
ungetbufptr : Integer;

function get_char : Char;
	begin
		Inc(yycolno);
		if (ungetbufptr > 0) then
		begin
			Dec(ungetbufptr);
			get_char := ungetbuf[ungetbufptr];
			exit;
		end;
		//
		if (bufptr = 0) then
		begin
			eoi := Length(yyinput_text);
			bufptr := 1;
		end;
		//
		if (bufptr <= eoi) then
		begin
			get_char := yyinput_text[bufptr];
			Inc(bufptr);
  ...
  begin
    if c=#0 then
      { ignore }
			...
			yyoutput[yyoutput.Count - 1] := yyoutput[yyoutput.Count - 1] + c;
	end(*put_char*);
